...
- 不再绘制插件自己的右侧问题横线，避免和 ChatGPT 网页端官方问题导航重复。
- 保留官方问题导航中部附近的插件菜单入口，可打开 Voyager 目录、导出等功能。
- 目录中仍可点击问题项跳转到对应用户问题；历史或弱锚点问题会先尝试加载并验证目标位置。
...
- 支持“目录”和“导出”两个标签页。
- 目录页列出当前已加载的用户问题，并在每个问题下显示覆盖回答核心说明与最终结论的一句话概览。
- 支持搜索过滤问题文本。
- 点击目录项可跳转到对应问题位置。
...
- 导出页默认进入“问题”模式，列表复用目录问题索引，可导出全部已记录问题。
- 当长对话只加载了一部分 DOM 时，可通过实时采集、滚动经过历史段落或“深度采集”补全问题索引。
- 可切换到“对话”模式，选择当前页面已加载的 User/Assistant 消息。
- 支持选择要导出的问题或消息。
- 支持全选和清空选择。
- 支持复制 Markdown 到剪贴板。
- 支持下载 Markdown 文件。
...
打开 ChatGPT 对话页后，右侧问题横线由 ChatGPT 网页端官方功能负责显示和跳转。插件不会再额外绘制一套横线。

- 点击官方横线：使用 ChatGPT 官方跳转。
- 点击或悬停官方导航中部附近的 Voyager 菜单入口：打开插件目录和导出面板。
- 点击插件目录项：跳转到对应用户问题；如果该历史问题暂未加载，插件会尝试按 turn、缓存锚点和索引位置找回。
- 如果对话内容继续生成或加载更多历史消息，插件目录会自动刷新。
...
- 问题模式导出目录索引中已记录的问题，适合长对话问题清单整理；未完整时先滚动经过历史段落或点击“深度采集”。
- 对话模式导出当前页面已加载的消息；未滚动加载出来的历史回复不会被 PDF/图片导出。
...
如果你遇到问题或希望提出新功能，可以在 GitHub Issues 中反馈：

- Bug report：使用 Bug report 模板。
- Feature request：使用 Feature request 模板。

反馈定位问题时，请尽量提供：

- 点击的是官方右侧横线、Voyager 菜单入口还是目录列表项；
- 期望跳转到哪里；
- 实际跳转到了哪里；
- 对话大概有多少轮；
- 浏览器版本和插件版本；
